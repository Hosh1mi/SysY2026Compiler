...
public:
    ...
    bool run(MachineFunction &function, MachineAnalysisManager &analyses) override;
};
...
class Arm64CFGOptimizationPass final : public Arm64MachineFunctionPass {
public:
    const char *name() const override { return "arm64-cfg-optimization"; }
    bool run(MachineFunction &function, MachineAnalysisManager &analyses) override;
};

class Arm64BitBranchOptimizationPass final : public Arm64MachineFunctionPass {
public:
    const char *name() const override { return "arm64-bit-branch-optimization"; }
    bool run(MachineFunction &function, MachineAnalysisManager &analyses) override;
};
