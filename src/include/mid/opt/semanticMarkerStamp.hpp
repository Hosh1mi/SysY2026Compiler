...
//     GVN 可跨内存状态 CSE。
//   - FnPure / FnReadOnly / ArgReadOnly：从 BasicAliasAnalysis 的函数摘要持久化到
//     Function / Argument，主要用于 IR 可见性（消费者仍读 live AA）。
//
...
private:
    ...
    bool isImmutableObject(Value *base, LoopInfo &LI);
    // 局部对象地址是否逃逸（被传入调用 / 指针本身被写入内存 / 未知使用者）。
    bool addressEscapes(Value *obj);
