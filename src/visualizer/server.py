...
RUNS = ROOT / "src" / "visualizer" / ".runs"
RUNS.mkdir(exist_ok=True)
COMPILER_PATH = os.environ.get("SYSY_COMPILER_PATH", "build/compiler")

BACKEND_STAGES = [
    ("dag-legalization", "DAG legalization"),
    ("dag-combine", "DAG combining"),
    ("instruction-select", "Instruction selection"),
    ("machine-optimization", "Machine optimization"),
    ("phi-elimination", "PHI elimination"),
    ("pre-ra-schedule", "Pre-RA scheduling"),
    ("register-allocation", "Register allocation"),
    ("frame-lowering", "Frame lowering"),
    ("post-ra-optimization", "Post-RA optimization"),
    ("assembly", "Assembly emission"),
]

MID_MARKER = re.compile(r"^; === IR (Before|After) (.+?) ===\s*$")
BACKEND_MARKER = re.compile(
    r"^; === AARCH64 (Before|After) ([^ ]+) (.+?) ===\s*$"
)
...
def _compiler_command(env: dict[str, str], args: list[str]) -> list[str]:
    ...
    if candidate.is_absolute() and str(candidate).startswith(str(ROOT)):
        container_compiler = f"{relative_root}/{candidate.relative_to(ROOT)}"
    else:
        raise RuntimeError(
            "Compiler path must be inside the project when Docker fallback is used."
        )
    mapped_args = [
        f"{relative_root}/{Path(arg).relative_to(ROOT)}" if Path(arg).is_absolute() and str(arg).startswith(str(ROOT)) else arg
        for arg in args
    ]
    return [
        "docker",
...
def _blocks(stderr: str, marker: re.Pattern[str]) -> list[tuple[str, str, str]]:
    ...
    for line in stderr.splitlines(keepends=True):
        ...
        if match:
            ...
            if marker is BACKEND_MARKER:
                current = (match.group(1), f"{match.group(2)}\0{match.group(3)}", [])
        elif current and (
            line.startswith("; === ")
...
def parse_backend(stderr: str) -> dict[str, dict[str, str]]:
    stages: dict[str, dict[str, list[str]]] = {}
    for when, encoded, text in _blocks(stderr, BACKEND_MARKER):
        phase, _ = encoded.split("\0", 1)
        stages.setdefault(phase, {"Before": [], "After": []})[when].append(text)
    return {
        phase: {
            "before": "\n\n".join(values["Before"]),
            "after": "\n\n".join(values["After"]),
    ...
    with _CACHE_LOCK:
        ...
        stdout, stderr = run_compiler(
            ...
            ["-S", "--dump-ir"],
            {
                ...
                "DEBUG_AARCH64_REWRITE_DUMP_PHASES": "1",
            },
        )
        ...
        backend = parse_backend(stderr)
        for phase, label in BACKEND_STAGES:
            backend_item = backend.get(phase, {"before": "", "after": "", "changed": False})
            stages.append({
                "id": f"backend:{phase}",
                "kind": "backend",
                "name": label,
    ...
    def do_POST(self) -> None:
        ...
        try:
            ...
            if kind == "mid":
                ...
                if phase == "assembly":
                    snapshots = snapshot["backend"].get("post-ra-optimization", {"before": "", "after": ""})
                    item = {
