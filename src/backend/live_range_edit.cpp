...
MachineBasicBlock::InstrList::iterator findInstruction(
    MachineBasicBlock &block, MachineInstr *instruction) {
  return std::find_if(
...
int getOrCreateSpillSlot(MachineFunction &function, VReg reg,
                         RegClass regClass,
...
MachineInstr makeSpillStore(VReg reg, RegClass regClass, int slot,
                            bool kill) {
...
bool LiveRangeEdit::splitLocalGap(
  ...
  if (!interval || !plan.block || !plan.splitAfter ||
      !plan.resumeBefore || plan.estimatedBenefit <= plan.estimatedCost)
    return false;
  ...
  for (const LiveRangeOperand &reference : interval->operands) {
    if (reference.block != plan.block ||
        reference.slot < plan.resumeSlot)
      continue;
  ...
  if (plan.rematerialization.definition) {
    ...
    const int slot = getOrCreateSpillSlot(
        function, plan.parent, parentInfo.regClass, spillSlots);
