...
struct MachineExpressionKeyHash {
	std::size_t operator()(const MachineExpressionKey &key) const {
		std::size_t hash =
		    std::hash<unsigned>{}(static_cast<unsigned>(key.opcode));
		combineHash(hash, std::hash<unsigned>{}(
		                      static_cast<unsigned>(key.resultClass)));
		combineHash(
		    hash, std::hash<unsigned>{}(static_cast<unsigned>(key.resultType)));
		for (const ExpressionOperandKey &input : key.inputs) {
			combineHash(
			    hash, std::hash<unsigned>{}(static_cast<unsigned>(input.kind)));
			combineHash(hash, std::hash<std::uint64_t>{}(input.value));
			combineHash(hash, std::hash<unsigned>{}(
			                      static_cast<unsigned>(input.regClass)));
		}
		return hash;
	}
};
...
buildExpressionKey(const MachineInstr &instruction,
	...
	if (descriptor.explicitDefs != 1 || descriptor.terminator ||
	    descriptor.call || descriptor.mayLoad || descriptor.mayStore ||
	    descriptor.hasSideEffects || descriptor.pseudo ||
	    descriptor.setsFlags || descriptor.usesFlags ||
	...
	for (auto &block : function.blocks()) {
		...
		for (auto it = instructions.begin(); it != instructions.end();) {
			...
			if (it->opcode() == Opcode::MOVIv4Zero) {
				...
				key +=
				    ":v" + std::to_string(it->operands()[1].virtualRegister());
			...
			struct PairAttempt {
				...
				for (std::size_t i = 0; i < candidates.size(); ++i) {
					for (std::size_t j = i + 1; j < candidates.size(); ++j) {
						...
						VReg earlyData = 0;
						if (!load &&
						    lhs.instruction->operands()[0].isVirtualRegister())
						...
						if (load &&
						    lhs.instruction->operands()[0].isSameRegisterAs(
						...
						pair.addOperand(lower->operands()[0])
						    ...
						    .addOperand(MachineOperand::vreg(lhs.address.root,
						                                     RegClass::GPR64))
						...
						pair.addMemoryOperand(MachineMemOperand{
						    ...
						    pairBytes, align, nullptr, std::nullopt,
						    lowerOffset, false});

						auto replacement =
						    load ? lhs.instruction : rhs.instruction;
						auto removed = load ? rhs.instruction : lhs.instruction;
						*replacement = std::move(pair);
						if (load) {
							for (unsigned operand = 0; operand < 2; ++operand)
								if (replacement->operands()[operand]
								        .isVirtualRegister())
									function.registerInfo().setDefinition(
									    replacement->operands()[operand]
									        .virtualRegister(),
									    &*replacement);
						}
			...
			for (const MachineOperand &operand : instruction.operands())
				if (operand.isVirtualRegister() && !operand.isDef) {
					...
					if (!inserted && use->second != &instruction)
						use->second = nullptr;
					auto [block, blockInserted] =
					    useBlock.emplace(reg, owned.get());
...
bool PostPhiConstantHoisting::run(MachineFunction &function) {
	...
	for (const MachineLoop &naturalLoop : loopInfo.loops()) {
		...
		for (MachineBasicBlock *predecessor : loop.header->predecessors())
			if (!loop.blocks.count(predecessor))
				outside.push_back(predecessor);
		if (outside.size() == 1 && outside.front()->successors().size() == 1)
	...
	for (const auto &[reg, info] : function.registerInfo().virtualRegisters()) {
		(void)info;
		if (MachineBasicBlock *block = registers.uniqueDefinitionBlock(reg))
		...
		while (localChange) {
			...
			for (MachineBasicBlock *block : blocks) {
				...
				for (auto it = instructions.begin();
					...
					if (nestedPreheaders.count(block) && block != loop.header)
						continue;
					bool invariant = true;
					for (const MachineOperand &operand : current->operands()) {
						if (operand.isPhysicalRegister()) {
							invariant = false;
							break;
						}
						if (!operand.isVirtualRegister() || operand.isDef)
							continue;
						auto definition =
						    definitionBlock.find(operand.virtualRegister());
...
bool AArch64ConditionOptimizer::run(MachineFunction &function) {
	...
	for (auto &block : function.blocks()) {
		auto &instructions = block->instructions();
		for (auto set = instructions.begin(); set != instructions.end();) {
			if (set->opcode() != Opcode::CSETW || set->operands().size() < 2 ||
			    !set->operands()[0].isVirtualRegister() ||
			    !set->operands()[0].isDef) {
				++set;
				continue;
			}
			VReg conditionReg = set->operands()[0].virtualRegister();
			...
			while (booleanCompare != instructions.end()) {
				...
				bool clobbersFlags =
				    descriptor.setsFlags || booleanCompare->isCall();
				for (const MachineOperand &operand : booleanCompare->operands())
					clobbersFlags |=
					    operand.isPhysicalRegister() && operand.isDef &&
					    operand.physicalRegister() == PhysReg::NZCV;
				if (clobbersFlags || booleanCompare->isTerminator())
					break;
				++booleanCompare;
			}
			...
			CondCode originalCondition = set->operands()[1].condition();
			bool rewroteSelect = false;
			for (auto select = std::next(booleanCompare);
			     select != instructions.end();) {
				bool candidate = (select->opcode() == Opcode::CSELW ||
			...
			function.registerInfo().eraseVirtualRegister(conditionReg);
			changed = true;
		}
	}
	...
	for (auto &block : function.blocks()) {
		auto &instructions = block->instructions();
		for (auto it = instructions.begin(); it != instructions.end();) {
			if (it->opcode() != Opcode::CSETW || it->operands().size() < 2 ||
			    !it->operands()[0].isVirtualRegister() ||
			    !it->operands()[0].isDef) {
				++it;
				continue;
			}
			VReg conditionReg = it->operands()[0].virtualRegister();
			...
			while (branch != instructions.end()) {
				const InstrDesc &descriptor = InstrInfo::get(branch->opcode());
				bool clobbersFlags = descriptor.setsFlags || branch->isCall();
				...
				bool candidate =
				    ...
				    branch->operands()[0].virtualRegister() == conditionReg;
				if (candidate)
					break;
			...
			MachineBasicBlock *target = branch->operands()[1].basicBlock();
			branch->setOpcode(Opcode::Bcc);
			branch->operands().clear();
			branch->addOperand(MachineOperand::condition(condition))
...
bool AArch64BranchFolding::run(MachineFunction &function) {
	...
	for (auto &owned : function.blocks()) {
		...
		while (localChange) {
			...
			for (const auto &useBlock : function.blocks())
				for (const MachineInstr &instruction : useBlock->instructions())
					for (const MachineOperand &operand : instruction.operands())
						if (operand.isVirtualRegister() && !operand.isDef)
			...
			for (auto bitAnd = instructions.begin();
				...
				VReg extracted = bitAnd->operands()[0].virtualRegister();
				...
				if (compare == instructions.end() ||
				    ...
				    !sameVReg(compare->operands()[0], bitAnd->operands()[0]) ||
				...
				if (branch == instructions.end() ||
					...
					continue;
				CondCode condition = branch->operands()[0].condition();
				if (condition != CondCode::EQ && condition != CondCode::NE)
					continue;
				if (flagsUsedAfter(owned.get(), std::next(branch)))
					continue;
				...
				if (bitAnd->opcode() == Opcode::ANDWri) {
					if (!bitAnd->operands()[1].isVirtualRegister() ||
					    bitAnd->operands()[2].kind() !=
					        MachineOperand::Kind::Immediate)
						continue;
					source = bitAnd->operands()[1];
					constantMask =
					    MaterializedI32{nullptr, 0,
				...
				bool oneBitMask =
				    constantMask && constantMask->value != 0 &&
				    (constantMask->value & (constantMask->value - 1)) == 0;
				std::int64_t maskImmediate =
				    constantMask
				        ? static_cast<std::int64_t>(constantMask->value)
				        : 0;
				bool immediateTest =
				    constantMask && !oneBitMask &&
				    InstrInfo::acceptsImmediate(Opcode::TSTWri, maskImmediate);

				if (oneBitMask) {
					unsigned bit = 0;
					while (((constantMask->value >> bit) & 1U) == 0)
						++bit;
					MachineBasicBlock *target =
					    branch->operands()[1].basicBlock();
					branch->setOpcode(condition == CondCode::EQ ? Opcode::TBZ
					                                            : Opcode::TBNZ);
					branch->operands().clear();
					branch->addOperand(source)
					    .addOperand(MachineOperand::immediate(bit))
					    .addOperand(MachineOperand::block(target));
					instructions.erase(compare);
				} else {
					MachineInstr test(immediateTest ? Opcode::TSTWri
					                                : Opcode::TSTWrr);
					test.addOperand(source);
					if (immediateTest) {
						test.addOperand(
						    MachineOperand::immediate(maskImmediate));
					} else {
	...
	for (auto &owned : function.blocks()) {
		...
		for (auto compare = instructions.begin();
			...
			CondCode condition = branch->operands()[0].condition();
			if ((condition != CondCode::EQ && condition != CondCode::NE) ||
			    flagsUsedAfter(owned.get(), std::next(branch))) {
				++compare;
				continue;
			}
			...
			MachineBasicBlock *target = branch->operands()[1].basicBlock();
			branch->setOpcode(condition == CondCode::EQ ? Opcode::CBZ
			                                            : Opcode::CBNZ);
			branch->operands().clear();
...
bool AArch64ExactHalvingLoopOptimizer::run(MachineFunction &function) {
	...
	while (batched) {
		...
		for (auto &parityOwned : function.blocks()) {
			...
			MachineBasicBlock *even = bitBranch->operands()[2].basicBlock();
			...
			if (shift->opcode() != Opcode::ASRWri ||
			    ...
			    !sameVReg(stateCopy->operands()[1], shift->operands()[0]) ||
			...
			if (incrementOnEdges &&
			    (increment->opcode() != Opcode::ADDWri ||
			     ...
			     !sameVReg(countCopy->operands()[1], increment->operands()[0])))
				continue;

			MachineBasicBlock *latch = toLatch->operands()[0].basicBlock();
			const std::size_t expectedLatchSize = incrementOnEdges ? 3 : 4;
			...
			if (increment->opcode() != Opcode::ADDWri ||
			    ...
			    compare->opcode() != Opcode::CMPWri ||
			    compare->operands().size() < 2 ||
			    !sameVReg(compare->operands()[0], stateCopy->operands()[0]) ||
			    compare->operands()[1].kind() !=
			        MachineOperand::Kind::Immediate ||
			    ...
			    exitBranch->operands()[0].kind() !=
			        MachineOperand::Kind::ConditionCode ||
			    exitBranch->operands()[0].condition() != CondCode::EQ ||
			...
			MachineBasicBlock *exit = exitBranch->operands()[1].basicBlock();
			...
			const MachineOperand &edgeCount = incrementOnEdges
			                                      ? countCopy->operands()[0]
			                                      : increment->operands()[0];
			auto continuationIt = continuation->instructions().begin();
			for (unsigned i = 0; i < 2; ++i, ++continuationIt) {
			...
			bool canReuseLoopCount = incrementOnEdges &&
			                         continuationCountCopy && exitCountCopy &&
			                         edgeCountUses == 2;
			...
			if (parityFallthrough->operands().size() == 1 &&
				...
				for (MachineBasicBlock *predecessor : latch->predecessors()) {
					...
					if (oddIncrementOnEdge != incrementOnEdges)
						continue;
					auto oddIncrement = predecessor->instructions().begin();
					auto update = oddIncrementOnEdge ? std::next(oddIncrement)
					                                 : oddIncrement;
					auto oddCountCopy = std::next(update);
					auto copy = oddIncrementOnEdge ? std::next(oddCountCopy)
					                               : std::next(update);
					...
					if (update->opcode() != Opcode::ADDWri ||
					    ...
					    !sameVReg(copy->operands()[0],
					              stateCopy->operands()[0]) ||
					    !sameVReg(copy->operands()[1], update->operands()[0]) ||
					    branch->opcode() != Opcode::B ||
					    branch->operands().size() != 1 ||
					    branch->operands()[0].kind() !=
					        MachineOperand::Kind::BasicBlock ||
					    branch->operands()[0].basicBlock() != latch ||
					    predecessor->predecessors().size() != 1 ||
					    predecessor->predecessors()[0] != oddCompute)
						continue;

					bool computesThreeTimesOdd = false;
					for (const MachineInstr &instruction :
					     oddCompute->instructions())
						if (instruction.opcode() == Opcode::ADDWlsl &&
						    instruction.operands().size() == 4 &&
						    sameVReg(instruction.operands()[0],
						             update->operands()[1]) &&
						    sameVReg(instruction.operands()[1], state) &&
						    sameVReg(instruction.operands()[2], state) &&
						    instruction.operands()[3].kind() ==
						        MachineOperand::Kind::Immediate &&
						    instruction.operands()[3].immediate() == 1)
			...
			VReg reversed = function.registerInfo().createVirtualRegister(
			    RegClass::GPR32, ValueType::I32);
			MachineInstr reverse(Opcode::RBITW);
			reverse
			    .addOperand(
			        MachineOperand::vreg(reversed, RegClass::GPR32, true))
			    .addOperand(state);
			auto reversedDefinition =
			    even->instructions().insert(shift, std::move(reverse));
			function.registerInfo().setDefinition(reversed,
			                                      &*reversedDefinition);

			VReg shiftAmount = function.registerInfo().createVirtualRegister(
			    RegClass::GPR32, ValueType::I32);
			MachineInstr countZeros(Opcode::CLZW);
			countZeros
			    .addOperand(
			        MachineOperand::vreg(shiftAmount, RegClass::GPR32, true))
			    .addOperand(MachineOperand::vreg(reversed, RegClass::GPR32));
			auto countDefinition =
			    even->instructions().insert(shift, std::move(countZeros));
			function.registerInfo().setDefinition(shiftAmount,
			                                      &*countDefinition);

			shift->setOpcode(Opcode::ASRWrr);
			shift->operands()[2] =
			    MachineOperand::vreg(shiftAmount, RegClass::GPR32);
			    ...
			    .addOperand(
			        canReuseLoopCount
			            ? MachineOperand::vreg(
			                  ...
			                  increment->operands()[1].regClass(), true)
			            : increment->operands()[0])
			    .addOperand(increment->operands()[1])
			    .addOperand(MachineOperand::vreg(shiftAmount, RegClass::GPR32));
			if (incrementOnEdges)
			...
			if (oddContinuation) {
				...
				auto oddIncrement = oddContinuation->instructions().begin();
				auto update =
				    oddIncrementOnEdge ? std::next(oddIncrement) : oddIncrement;
				auto oddCountCopy = std::next(update);
				auto copy = oddIncrementOnEdge ? std::next(oddCountCopy)
				                               : std::next(update);
				auto oldBranch = std::next(copy);
				auto oddBatchInsert = oddIncrementOnEdge ? oddCountCopy : copy;
				MachineOperand updatedState = MachineOperand::vreg(
				    update->operands()[0].virtualRegister(),
				    update->operands()[0].regClass());

				VReg oddReversed =
				    function.registerInfo().createVirtualRegister(
				        RegClass::GPR32, ValueType::I32);
				MachineInstr reverseOdd(Opcode::RBITW);
				reverseOdd
				    .addOperand(MachineOperand::vreg(oddReversed,
				                                     RegClass::GPR32, true))
				    .addOperand(updatedState);
				auto reverseOddDefinition =
				    oddContinuation->instructions().insert(
				        oddBatchInsert, std::move(reverseOdd));
				function.registerInfo().setDefinition(oddReversed,
				                                      &*reverseOddDefinition);
				...
				countOddZeros
				    .addOperand(MachineOperand::vreg(oddShiftAmount,
				                                     RegClass::GPR32, true))
				    .addOperand(
				        MachineOperand::vreg(oddReversed, RegClass::GPR32));
				auto oddCountDefinition =
				    oddContinuation->instructions().insert(
				        oddBatchInsert, std::move(countOddZeros));
				function.registerInfo().setDefinition(oddShiftAmount,
				                                      &*oddCountDefinition);

				MachineOperand shiftedState = copy->operands()[0];
				copy->setOpcode(Opcode::ASRWrr);
				copy->operands().clear();
				copy->addOperand(shiftedState)
				    .addOperand(updatedState)
				    .addOperand(
				        MachineOperand::vreg(oddShiftAmount, RegClass::GPR32));
				oddContinuation->instructions().erase(oldBranch);

				if (oddIncrementOnEdge) {
					MachineOperand countDestination =
					    canReuseLoopCount
					        ? MachineOperand::vreg(
					              increment->operands()[1].virtualRegister(),
					              increment->operands()[1].regClass(), true)
					...
					oddCountCopy->addOperand(countDestination)
					...
					addShifts
					    .addOperand(MachineOperand::vreg(batchedCount,
					                                     RegClass::GPR32, true))
					    .addOperand(increment->operands()[1])
					    .addOperand(MachineOperand::vreg(oddShiftAmount,
					                                     RegClass::GPR32));
					MachineInstr &addShiftsDefinition =
					    oddContinuation->append(std::move(addShifts));
					function.registerInfo().setDefinition(batchedCount,
					                                      &addShiftsDefinition);

					MachineInstr addOddIteration(Opcode::ADDWri);
					addOddIteration.addOperand(increment->operands()[0])
					    .addOperand(
					        MachineOperand::vreg(batchedCount, RegClass::GPR32))
					    .addOperand(MachineOperand::immediate(1));
					oddContinuation->append(std::move(addOddIteration));
				}
			...
			if (canReuseLoopCount) {
				...
				for (auto copyIt = continuation->instructions().begin();
				     copyIt != continuation->instructions().end(); ++copyIt) {
