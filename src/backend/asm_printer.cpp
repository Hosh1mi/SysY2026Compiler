...
std::string blockLabel(const MachineFunction &function,
	...
	return ".L" + function.name() + "_bb" + std::to_string(block->number());
}
...
void emitGlobalMemory(std::ostream &output, const char *mnemonic,
	...
	output << '\t' << mnemonic << ' ' << value << ", ["
	       ...
	       << ", :lo12:" << instruction.operands()[2].symbol() << "]\n";
}
...
void printInstruction(const MachineFunction &function,
                      const MachineInstr &instruction, std::ostream &output) {
	...
	case Opcode::COPY: {
		...
		if (operands[0].regClass() == RegClass::NEON128)
			output << "\tmov " << destination << ".16b, " << source << ".16b\n";
	...
	case Opcode::MOVK: {
		if (operands.size() != 4 || !operands[0].isPhysicalRegister() ||
	...
	case Opcode::MOVIv4Zero:
		output << "\tmovi " << registerName(operands[0]) << ".4s, #0\n";
		break;
	case Opcode::MOVIv4s: {
		if (operands.size() != 3 ||
		    operands[1].kind() != MachineOperand::Kind::Immediate ||
	...
	case Opcode::ADDlow:
		output << "\tadd " << registerName(operands[0]) << ", "
		       ...
		       << ", :lo12:" << operands[2].symbol() << '\n';
		break;
	case Opcode::ADDWrr:
		emitThreeRegisters(output, "add", instruction);
		break;
	case Opcode::SUBWrr:
	...
	case Opcode::ADDWrs:
		output << "\tadd " << registerName(operands[0]) << ", "
		       << registerName(operands[1]) << ", " << registerName(operands[2])
		       << ", lsr #" << operands[3].immediate() << '\n';
		break;
	case Opcode::ADDWrsX:
		output << "\tadd " << registerNameAs(operands[0], RegClass::GPR32)
		       << ", " << registerNameAs(operands[1], RegClass::GPR32) << ", "
		       << registerNameAs(operands[2], RegClass::GPR32) << ", lsr #"
		       << operands[3].immediate() << '\n';
		break;
	case Opcode::ADDWlsl:
		output << "\tadd " << registerName(operands[0]) << ", "
		       << registerName(operands[1]) << ", " << registerName(operands[2])
		       << ", lsl #" << operands[3].immediate() << '\n';
		break;
	case Opcode::NEGW:
		...
		break;
	case Opcode::MULWrr:
		emitThreeRegisters(output, "mul", instruction);
		break;
	case Opcode::MULXrr:
		emitThreeRegisters(output, "mul", instruction);
		break;
	case Opcode::SDIVWrr:
		emitThreeRegisters(output, "sdiv", instruction);
		break;
	case Opcode::UDIVWrr:
		emitThreeRegisters(output, "udiv", instruction);
		break;
	case Opcode::UDIVXrr:
		emitThreeRegisters(output, "udiv", instruction);
		break;
	...
	case Opcode::SMADDLXrrr:
		output << "\tsmaddl " << registerNameAs(operands[0], RegClass::GPR64)
		       << ", " << registerNameAs(operands[1], RegClass::GPR32) << ", "
		       << registerNameAs(operands[2], RegClass::GPR32) << ", "
	...
	case Opcode::NEGX:
		...
		break;
	case Opcode::ANDWrr:
		emitThreeRegisters(output, "and", instruction);
		break;
	case Opcode::ORRWrr:
		emitThreeRegisters(output, "orr", instruction);
		break;
	case Opcode::ORRWri:
	case Opcode::ORRXri:
		if (operands.size() != 3 ||
		...
		output << "\torr " << registerName(operands[0]) << ", "
		       ...
		       << (instruction.opcode() == Opcode::ORRWri
		               ? static_cast<std::uint64_t>(static_cast<std::uint32_t>(
		                     operands[2].immediate()))
		               : static_cast<std::uint64_t>(operands[2].immediate()))
		       << std::dec << '\n';
		break;
	case Opcode::EORWrr:
		emitThreeRegisters(output, "eor", instruction);
		break;
	case Opcode::ANDXrr:
		emitThreeRegisters(output, "and", instruction);
		break;
	...
	case Opcode::EORXrr:
		...
		break;
	case Opcode::LSLWrr:
		emitThreeRegisters(output, "lsl", instruction);
		break;
	case Opcode::LSRWrr:
		emitThreeRegisters(output, "lsr", instruction);
		break;
	case Opcode::ASRWrr:
		emitThreeRegisters(output, "asr", instruction);
		break;
	case Opcode::LSLXrr:
		emitThreeRegisters(output, "lsl", instruction);
		break;
	...
	case Opcode::LSLWri:
		...
		break;
	case Opcode::LSRWri:
		emitThreeWithImmediate(output, "lsr", instruction);
		break;
	case Opcode::ASRWri:
	...
	case Opcode::COPYXtoW:
		if (!RegisterInfo::aliases(operands[0].physicalRegister(),
			...
			output << "\tmov " << registerNameAs(operands[0], RegClass::GPR32)
			       << ", " << registerNameAs(operands[1], RegClass::GPR32)
			       << '\n';
		break;
	case Opcode::SUBSPri:
	...
	case Opcode::RBITW:
		...
		break;
	case Opcode::FADDS:
		emitThreeRegisters(output, "fadd", instruction);
		break;
	case Opcode::FSUBS:
		emitThreeRegisters(output, "fsub", instruction);
		break;
	case Opcode::FMULS:
		emitThreeRegisters(output, "fmul", instruction);
		break;
	case Opcode::FDIVS:
		emitThreeRegisters(output, "fdiv", instruction);
		break;
	case Opcode::FNEGS:
	...
	case Opcode::STPXpre:
		output << "\tstp " << registerName(operands[0]) << ", "
		       << registerName(operands[1]) << ", ["
		       << registerName(operands[2]) << ", #" << operands[3].immediate()
		       << "]!\n";
		break;
	...
	case Opcode::STPDi:
	case Opcode::STPQi: {
		bool qPair = instruction.opcode() == Opcode::LDPQi ||
		             instruction.opcode() == Opcode::STPQi;
		bool dPair = instruction.opcode() == Opcode::LDPDi ||
		             instruction.opcode() == Opcode::STPDi;
		unsigned scale = qPair ? 16U
		                 : (dPair || instruction.opcode() == Opcode::LDPXi ||
		                    instruction.opcode() == Opcode::STPXi)
		                     ? 8U
		                     : 4U;
	...
	case Opcode::CBZ:
	case Opcode::CBNZ:
		output << '\t' << (instruction.opcode() == Opcode::CBZ ? "cbz" : "cbnz")
		       << ' ' << registerName(operands[0]) << ", "
		       << blockLabel(function, operands[1].basicBlock()) << '\n';
		break;
	case Opcode::TBZ:
	case Opcode::TBNZ:
		output << '\t' << (instruction.opcode() == Opcode::TBZ ? "tbz" : "tbnz")
		       << ' ' << registerName(operands[0]) << ", #"
		       << operands[1].immediate() << ", "
	...
	case Opcode::INSv4f32:
		...
		if (instruction.opcode() == Opcode::INSv4f32)
			output << registerNameAs(operands[2], RegClass::NEON128)
			       << ".s[0]\n";
		else
	...
	case Opcode::NEGv4f32:
		output << '\t'
		       << (instruction.opcode() == Opcode::NEGv4i32 ? "neg" : "fneg")
		       << ' ' << vectorView(operands[0]) << ", "
	...
	case Opcode::MVNv16i8:
		output << "\tmvn " << registerName(operands[0]) << ".16b, "
		       << registerName(operands[1]) << ".16b\n";
		break;
	case Opcode::CMEQv4i32:
	case Opcode::CMGTv4i32:
	case Opcode::CMGEv4i32:
	case Opcode::CMHIv4i32:
	case Opcode::CMHSv4i32:
	case Opcode::FCMEQv4f32:
	case Opcode::FCMGTv4f32:
	case Opcode::FCMGEv4f32:
		output << '\t'
	...
	case Opcode::USHLv4i32:
		output << '\t'
		       << (instruction.opcode() == Opcode::SSHLv4i32 ? "sshl" : "ushl")
		       << ' ' << vectorView(operands[0]) << ", "
		       << vectorView(operands[1]) << ", " << vectorView(operands[2])
		       << '\n';
		break;
		...
		output << '\t'
		       << (instruction.opcode() == Opcode::MLAv4i32    ? "mla"
		           ...
		           : instruction.opcode() == Opcode::FMLAv4f32 ? "fmla"
		                                                       : "fmls")
		       << ' ' << vectorView(operands[0]) << ", "
		       << vectorView(operands[2]) << ", " << vectorView(operands[3])
		       << '\n';
		break;
...
void AArch64AssemblyPrinter::printFunction(const MachineFunction &function,
	...
	output << "\t.size " << function.name() << ", .-" << function.name()
	       << "\n";
