...
SignedDivisorInfo analyzeSignedDivisor(std::int32_t divisor) {
	...
	info.reducible = true;
	info.magnitude = divisor < 0 ? 0U - static_cast<std::uint32_t>(divisor)
...
MagicNumber computeSignedMagic(std::int32_t divisor) {
	...
	std::uint32_t signAdjusted =
	    0x80000000U + (static_cast<std::uint32_t>(divisor) >> 31);
	std::uint32_t cutoff = signAdjusted - 1 - signAdjusted % absoluteDivisor;
	std::uint32_t quotient1 = 0x80000000U / cutoff;
	std::uint32_t remainder1 = 0x80000000U - quotient1 * cutoff;
	std::uint32_t quotient2 = 0x80000000U / absoluteDivisor;
	std::uint32_t remainder2 = 0x80000000U - quotient2 * absoluteDivisor;
	...
	std::int32_t multiplier = static_cast<std::int32_t>(quotient2 + 1);
	if (divisor < 0)
...
BarrettModulus64 computeBarrettModulus64(std::uint32_t modulus) {
	...
	info.mu = static_cast<std::uint64_t>((static_cast<__uint128_t>(1) << 64) /
	                                     modulus);
	return info;
}
