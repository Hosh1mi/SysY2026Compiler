...
bool colorIsLegal(const LiveInterval &interval, PhysReg color,
	...
	if (forbidden != forbiddenColors.end() && forbidden->second.count(color))
		return false;
...
bool isSoleColorBlocker(VReg spill, VReg blocker, PhysReg color,
...
bool intervalInside(const LiveInterval &interval, MachineSlotRange range) {
...
LiveRangeSplitPlans LiveRangeSplitAnalysis::analyze(
	...
	for (const auto &[blocker, color] : assignments) {
		...
		const double boundaryCost =
		    blockWeight(*block) *
		    (canRematerialize ? rematerialization->second.cost
		...
		for (std::size_t gapIndex = 0;
			...
			const LiveRangeOperand &right =
			    blockerInterval.operands[gapIndex + 1];
			...
			for (VReg spill : spills) {
				...
				if (!intervalInside(spillInterval, gap) ||
				    ...
				    !isSoleColorBlocker(spill, blocker, color, graph,
				                        assignments))
					continue;
	...
	for (LocalSplitPlan &plan : localCandidates) {
		...
		relieved.insert(plan.relievedSpills.begin(), plan.relievedSpills.end());
