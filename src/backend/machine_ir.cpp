...
bool MachineInstr::isBranch() const { return InstrInfo::get(opcode_).branch; }

bool MachineInstr::isCall() const { return InstrInfo::get(opcode_).call; }

bool MachineInstr::mayLoad() const { return InstrInfo::get(opcode_).mayLoad; }

bool MachineInstr::mayStore() const { return InstrInfo::get(opcode_).mayStore; }

bool MachineInstr::hasSideEffects() const {
	return InstrInfo::get(opcode_).hasSideEffects;
}

bool MachineInstr::isPseudo() const { return InstrInfo::get(opcode_).pseudo; }
...
VReg MachineRegisterInfo::createVirtualRegister(RegClass regClass,
	...
	virtualRegisters_.emplace(reg,
	                          VRegInfo{regClass, valueType, nullptr, false, 0});
	return reg;
}
...
int MachineFrameInfo::createFixedObject(unsigned size, int offset,
	...
	int index = static_cast<int>(objects_.size());
	objects_.push_back(
	    StackObject{index, size, alignment, offset, false, true});
	maxAlignment = std::max(maxAlignment, alignment);
	return index;
}
...
const std::string &MachineFunction::getOrCreateVectorConstant(
	...
	entry.lanes = lanes;
	entry.label =
	    ".LCPI_" + name_ + "_" + std::to_string(vectorConstantPool_.size());
...
std::string operandText(const MachineOperand &operand) {
	...
	case MachineOperand::Kind::PhysicalRegister:
		output << '$'
		       << RegisterInfo::name(operand.physicalRegister(),
		                             operand.regClass());
		break;
