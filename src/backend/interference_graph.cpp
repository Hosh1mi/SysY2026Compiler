...
bool InterferenceGraph::addEdge(VReg lhs, VReg rhs) {
	...
	const std::size_t lhsWord = lhsIndex * wordsPerRow_ + rhsIndex / 64;
	const std::uint64_t rhsBit = std::uint64_t{1} << (rhsIndex % 64);
	if (edges_[lhsWord] & rhsBit)
		return false;

	edges_[lhsWord] |= rhsBit;
	edges_[rhsIndex * wordsPerRow_ + lhsIndex / 64] |= std::uint64_t{1}
	                                                   << (lhsIndex % 64);
	++degrees_[lhsIndex];
	++degrees_[rhsIndex];
	return true;
...
InterferenceGraph::NeighborRange InterferenceGraph::neighbors(VReg reg) const {
