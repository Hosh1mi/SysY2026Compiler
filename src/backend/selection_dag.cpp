...
SDOpcode binaryOpcode(Instruction::OpID opcode) {
	...
	case Instruction::SDiv:
		return SDOpcode::SDiv;
	case Instruction::SRem:
		return SDOpcode::SRem;
	case Instruction::UDiv:
		return SDOpcode::UDiv;
	case Instruction::URem:
		return SDOpcode::URem;
	case Instruction::FAdd:
		return SDOpcode::FAdd;
	case Instruction::FSub:
		return SDOpcode::FSub;
	case Instruction::FMul:
		return SDOpcode::FMul;
	case Instruction::FDiv:
		return SDOpcode::FDiv;
	case Instruction::Shl:
		return SDOpcode::Shl;
	case Instruction::LShr:
		return SDOpcode::LShr;
	case Instruction::AShr:
		return SDOpcode::AShr;
...
bool isSupportedValueType(ValueType type) {
	return type == ValueType::I1 || type == ValueType::I32 ||
	       type == ValueType::I64 || type == ValueType::F32 ||
	       type == ValueType::Ptr || type == ValueType::V4I32 ||
	       type == ValueType::V4F32 || type == ValueType::Flags;
}
...
bool callArgsFitInRegisters(Instruction *call) {
	...
	for (unsigned i = 0; i + 1 < call->num_ops(); ++i) {
		ValueType type =
		    SelectionDAGBuilder::valueType(call->get_operand(i)->type_);
		bool vectorBank = type == ValueType::F32 || type == ValueType::V4F32 ||
...
bool matchFullSplat(SDNode &root,
	...
	for (unsigned depth = 0; depth < 4; ++depth) {
		if (!current || current->opcode() != SDOpcode::InsertElement ||
		    current->operands().size() != 3 || current->resultTypes().empty() ||
...
SDValue SelectionDAGBuilder::getValue(FunctionDAG &functionDAG,
	...
	if (auto *vector = dynamic_cast<ConstantVector *>(value)) {
		...
		for (Constant *element : vector->elements_) {
			...
			else if (auto *constant = dynamic_cast<ConstantFloat *>(element)) {
				std::uint32_t bits = 0;
	...
	if (auto *global = dynamic_cast<GlobalVariable *>(value)) {
		SDNode &node =
		    dag.createNode(SDOpcode::GlobalAddress, {ValueType::Ptr});
		...
		for (Instruction *instruction : block->instr_list_) {
			...
			for (unsigned i = 0; i < phi->num_ops(); i += 2) {
				...
				if (dynamic_cast<Constant *>(value) ||
					...
					operands[i / 2] = getValue(*result, entryDAG, value);
			}
		}
	}
	...
	for (BasicBlock *block : result->blockOrder) {
		...
		for (Instruction *instruction : block->instr_list_) {
			...
			if (binary != SDOpcode::Invalid) {
				created =
				    &dag.createNode(binary, {valueType(instruction->type_)},
				                    {operand(0), operand(1)});
			} else {
				switch (instruction->op_id_) {
				case Instruction::FNeg:
					created = &dag.createNode(SDOpcode::FNeg,
				...
				case Instruction::ICmp: {
					created = &dag.createNode(SDOpcode::ICmp,
					                          {valueType(instruction->type_)},
					                          {operand(0), operand(1)});
					created->predicate = static_cast<int>(
				...
				case Instruction::GetElementPtr: {
					created = &dag.createNode(SDOpcode::GEP, {ValueType::Ptr},
					                          {operand(0)});
					Type *current = static_cast<PointerType *>(
				...
				case Instruction::Store: {
					...
					created =
					    &dag.createNode(SDOpcode::Store, {ValueType::Invalid},
				...
				case Instruction::ZExt:
					created = &dag.createNode(SDOpcode::ZExt,
					                          {valueType(instruction->type_)},
					                          {operand(0)});
					break;
				case Instruction::SExt:
					created = &dag.createNode(SDOpcode::SExt,
					                          {valueType(instruction->type_)},
					                          {operand(0)});
					break;
				case Instruction::Trunc:
					created = &dag.createNode(SDOpcode::Trunc,
					                          {valueType(instruction->type_)},
					                          {operand(0)});
					break;
				case Instruction::FPtoSI:
					created = &dag.createNode(SDOpcode::FPToSI,
					                          {valueType(instruction->type_)},
					                          {operand(0)});
					break;
				case Instruction::SItoFP:
					created = &dag.createNode(SDOpcode::SIToFP,
					                          {valueType(instruction->type_)},
					                          {operand(0)});
					break;
				case Instruction::BitCast:
					created = &dag.createNode(SDOpcode::Bitcast,
					                          {valueType(instruction->type_)},
					                          {operand(0)});
					break;
				case Instruction::Clz:
					created = &dag.createNode(SDOpcode::Clz,
					                          {valueType(instruction->type_)},
					                          {operand(0)});
					break;
				case Instruction::InsertElement: {
					...
					if (valueType(instruction->get_operand(0)->type_) !=
					    resultType) {
						SDNode &undef =
						    dag.createNode(SDOpcode::Constant, {resultType});
						undef.shuffleMask.assign(4, 0);
						vector = SDValue{&undef, 0};
					}
					created =
					    &dag.createNode(SDOpcode::InsertElement, {resultType},
					                    {vector, operand(1), operand(2)});
					break;
				}
				case Instruction::ExtractElement:
					created = &dag.createNode(SDOpcode::ExtractElement,
					                          {valueType(instruction->type_)},
					                          {operand(0), operand(1)});
					break;
				case Instruction::ShuffleVector: {
					created = &dag.createNode(SDOpcode::ShuffleVector,
					                          {valueType(instruction->type_)},
					                          {operand(0), operand(1)});
					created->shuffleMask =
				...
				case Instruction::Call: {
					...
					bool emitTail =
					    callInst->is_tail() &&
					    callArgsFitInRegisters(callInst) && term &&
					    term->is_ret() &&
					...
					created =
					    &dag.createNode(SDOpcode::Call, std::move(results),
					                    std::move(operands));
					created->symbol = callee ? callee->name_ : std::string();
					chain = SDValue{created,
					                static_cast<unsigned>(
					                    created->resultTypes().size() - 1)};
					break;
				}
				case Instruction::Br: {
					if (instruction->num_ops() == 1) {
						created = &dag.createNode(
						    SDOpcode::Branch, {ValueType::Invalid}, {chain});
				...
				case Instruction::Ret: {
					...
					created =
					    &dag.createNode(SDOpcode::Return, {ValueType::Invalid},
					                    std::move(operands));
					chain = SDValue{created, 0};
					break;
				}
...
bool DAGCombiner::run(FunctionDAG &functionDAG,
	...
	for (BasicBlock *block : functionDAG.blockOrder) {
		for (const auto &owned : functionDAG.blocks.at(block)->nodes()) {
			SDNode &node = *owned;
			if (node.opcode() == SDOpcode::Add && node.operands().size() == 2 &&
			    (node.resultTypes().front() == ValueType::I32 ||
			     node.resultTypes().front() == ValueType::V4I32)) {
	...
	for (BasicBlock *block : functionDAG.blockOrder) {
		for (const auto &owned : functionDAG.blocks.at(block)->nodes()) {
			...
			if (match.base && match.base->opcode() == SDOpcode::Constant &&
