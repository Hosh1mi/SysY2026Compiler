...
MachineVerifier::verify(const MachineFunction &function) const {
	...
	for (std::size_t blockIndex = 0; blockIndex < function.blocks().size();
		...
		for (const auto &instruction : block->instructions()) {
			...
			if (function.hasProperty(MachineProperty::FrameFinalized)) {
				unsigned width = scaledMemoryWidth(instruction.opcode());
				if (width && instruction.operands().size() >= 3) {
					const MachineOperand &offset = instruction.operands()[2];
					if (offset.kind() != MachineOperand::Kind::Immediate ||
					    offset.immediate() < 0 ||
				...
				if (pairWidth && instruction.operands().size() >= 4) {
					const MachineOperand &offset = instruction.operands()[3];
					if (offset.kind() != MachineOperand::Kind::Immediate ||
		...
		for (const auto &block : function.blocks()) {
			...
			for (const MachineInstr &instruction : block->instructions()) {
				for (unsigned operandIndex = 0;
					...
					if (instruction.opcode() == Opcode::PHI) {
						if (operandIndex + 1 >= instruction.operands().size() ||
