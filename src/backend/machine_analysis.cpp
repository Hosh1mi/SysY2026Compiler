	...
	for (const auto &owned : function.blocks()) {
		...
		for (MachineInstr &instruction : block->instructions()) {
			for (unsigned index = 0; index < instruction.operands().size();
				...
				(operand.isDef ? definitions_
				               : uses_)[operand.virtualRegister()]
				    .push_back(reference);
			}
		}
	}
...
MachineBasicBlock *MachineRegisterIndex::uniqueDefinitionBlock(VReg reg) const {
	const auto &references = definitions(reg);
			...
			while (!worklist.empty()) {
				...
				for (MachineBasicBlock *predecessor : current->predecessors())
					if (dominators.isReachable(predecessor) &&
					    loop.blocks.insert(predecessor).second &&
					    predecessor != header)
