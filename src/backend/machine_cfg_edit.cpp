...
bool hasSuccessor(const MachineBasicBlock &predecessor,
  ...
  return std::find(predecessor.successors().begin(),
                   predecessor.successors().end(), &successor) !=
...
bool MachineCFGEdit::canSplitEdge(
...
MachineBasicBlock &MachineCFGEdit::splitEdge(
    MachineFunction &function, MachineBasicBlock &predecessor,
    MachineBasicBlock &successor, std::string name) {
  if (!canSplitEdge(predecessor, successor))
    throw std::logic_error("cannot split an implicit or missing Machine edge");
