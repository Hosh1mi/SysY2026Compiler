...
void MachineSlotIndexes::number(MachineFunction &function) {
  ...
  for (const auto &owned : function.blocks()) {
    ...
    blockRanges_.emplace(block,
                         MachineSlotRange{blockBegin, blockEnd});
    slot += kInstructionSpacing;
  }
}
...
MachineSlot MachineSlotIndexes::useSlot(
    const MachineInstr &instruction) const {
  return instruction.slotIndex + 1;
}

MachineSlot MachineSlotIndexes::defSlot(
    const MachineInstr &instruction) const {
  return instruction.slotIndex + 2;
}

MachineSlot MachineSlotIndexes::deadSlot(
    const MachineInstr &instruction) const {
  return instruction.slotIndex + 3;
}

MachineSlotRange MachineSlotIndexes::blockRange(
  ...
  for (const auto &owned : function.blocks()) {
    ...
    for (MachineInstr &instruction : block->instructions()) {
      for (unsigned index = 0; index < instruction.operands().size();
        ...
        interval.operands.push_back(
            LiveRangeOperand{block, &instruction, index, slot,
                             operand.isDef});
        if (!operand.isDef)
