...
bool isPairOffsetEncodable(std::int64_t offset, unsigned scale) {
    return offset % scale == 0 && offset / scale >= -64 &&
           offset / scale <= 63;
}

bool isScaledOffsetEncodable(std::int64_t offset, unsigned scale) {
    return offset >= 0 && offset % scale == 0 &&
           offset / scale <= 4095;
}
...
void addMemoryInfo(MachineInstr &instruction,
                   MachineMemOperand::Access access, unsigned size,
                   unsigned alignment, std::optional<int> frameIndex,
                   std::int64_t offset) {
        ...
        for (auto it = instructions.begin(); it != instructions.end();) {
            if (it->opcode() == Opcode::ADJCALLSTACKDOWN ||
                ...
                if (it->operands().size() != 1 ||
                    ...
                    throw std::logic_error(
                        "malformed call-stack adjustment");
            ...
            if (it->opcode() == Opcode::LEA_FRAME) {
                ...
                if (amount == 0) {
                    ...
                    it->addOperand(destination)
                        .addOperand(phys(base, RegClass::GPR64))
                        .addOperand(
                            MachineOperand::immediate(amount));
                }
                offset -= amount;
            ...
            if (it->opcode() == Opcode::SPILL_LOAD ||
                ...
                std::int64_t absoluteOffset =
                    object.fixed
                        ? static_cast<std::int64_t>(frame.stackSize) +
                              object.offset
                        : object.offset;
                std::int64_t memoryOffset = object.offset;
                PhysReg base =
                    object.fixed ? PhysReg::X29 : PhysReg::SP;
                if (!isScaledOffsetEncodable(
                        memoryOffset, width)) {
                    ...
                    while (remaining) {
                        ...
                        restore
                            ...
                            .addOperand(
                                MachineOperand::immediate(amount));
                        instructions.insert(
                            restorePosition, std::move(restore));
                        remaining -= amount;
                    }
                }
                it->operands().clear();
                it->addOperand(std::move(value))
                    .addOperand(phys(base, RegClass::GPR64))
                    .addOperand(
                        MachineOperand::immediate(memoryOffset));
                ++it;
                continue;
            }
            ++it;
    ...
    for (std::size_t i = 0; i < frame.savedRegisters.size();) {
        ...
        if (i + 1 < frame.savedRegisters.size()) {
            ...
            if (vector == nextVector &&
                ...
                MachineInstr save(vector ? Opcode::STPDi
                                         : Opcode::STPXi);
                save.addOperand(phys(
                                    reg, vector ? RegClass::NEON128
                                                : RegClass::GPR64))
                    .addOperand(phys(
                        next, vector ? RegClass::NEON128
                                     : RegClass::GPR64))
                    .addOperand(phys(PhysReg::SP, RegClass::GPR64))
                    .addOperand(MachineOperand::immediate(offset));
                addMemoryInfo(
                    save, MachineMemOperand::Access::Store,
                    size * 2, size, std::nullopt, offset);
                entry->instructions().insert(
                    insertion, std::move(save));
                i += 2;
                continue;
            }
        }
        MachineInstr save(vector ? Opcode::STRDui : Opcode::STRXui);
        save.addOperand(phys(reg, vector ? RegClass::NEON128
                                        : RegClass::GPR64))
            .addOperand(phys(PhysReg::SP, RegClass::GPR64))
            .addOperand(MachineOperand::immediate(
                frame.savedRegisterOffsets.at(reg)));
        addMemoryInfo(save, MachineMemOperand::Access::Store,
                      8, 8, std::nullopt,
                      frame.savedRegisterOffsets.at(reg));
        ...
        for (auto it = instructions.begin(); it != instructions.end(); ++it) {
            ...
            for (std::size_t i = frame.savedRegisters.size(); i > 0;) {
                if (i >= 2) {
                    ...
                    unsigned size = 8;
                    int offset =
                        frame.savedRegisterOffsets.at(first);
                ...
                MachineInstr restore(vector ? Opcode::LDRDui
                                            : Opcode::LDRXui);
                restore
                    .addOperand(phys(saved,
                                     vector ? RegClass::NEON128
                                            : RegClass::GPR64,
                                     true))
                    .addOperand(phys(PhysReg::SP, RegClass::GPR64))
            ...
            pop.addOperand(
                   phys(PhysReg::X29, RegClass::GPR64, true))
                .addOperand(
                    phys(PhysReg::X30, RegClass::GPR64, true))
                .addOperand(
                    phys(PhysReg::SP, RegClass::GPR64, true))
                .addOperand(MachineOperand::immediate(16));
