...
static std::string qMemReg(const ParsedLine &line) {
	if (line.operands.empty())
		return "";
	if ((line.mnemonic == "ldr" || line.mnemonic == "str") &&
	    regClass(line.operands[0]) == 'q')
		return line.operands[0];
	if ((line.mnemonic == "ld1" || line.mnemonic == "st1") &&
	    line.operands[0].size() >= 6 && line.operands[0].front() == '{' &&
	    line.operands[0].back() == '}') {
		std::string text = trim(line.operands[0].substr(1, line.operands[0].size() - 2));
		if (text.size() < 4 || text[0] != 'v')
			return "";
		size_t dot = text.find('.');
		if (dot == std::string::npos)
			return "";
		std::string suffix = text.substr(dot);
		if (suffix != ".16b" && suffix != ".8h" && suffix != ".4s" &&
		    suffix != ".2d")
			return "";
		return "q" + text.substr(1, dot - 1);
	}
	return "";
}

static bool parseAddSubImmediate(const ParsedLine &line, const std::string &dst,
                                 std::string &src, int &delta) {
	if ((line.mnemonic != "add" && line.mnemonic != "sub") ||
	    line.operands.size() != 3 || line.operands[0] != dst)
		return false;
	int imm = 0;
	if (!parseHashImmediate(line.operands[2], imm))
		return false;
	src = line.operands[1];
	delta = line.mnemonic == "add" ? imm : -imm;
	return regClass(src) == 'x';
}

static bool traceAddressToCurrentBase(const MachineBasicBlock &block,
                                      size_t beforeIdx,
                                      const MemOperand &mem,
                                      const std::string &targetBase,
                                      int &targetOffset) {
	if (!mem.valid || mem.base.empty() || targetBase.empty() ||
	    targetBase == "sp")
		return false;

	std::string root = mem.base;
	...
	const size_t begin = beforeIdx > 12 ? beforeIdx - 12 : 0;
	for (size_t i = beforeIdx; i-- > begin;) {
		ParsedLine line = parseLine(block.instrs[i].text);
		if (line.kind != LineKind::Instruction)
			break;

		if (line.mnemonic == "mov" && line.operands.size() == 2 &&
		    line.operands[0] == targetBase && line.operands[1] == root) {
			targetOffset = offset;
			return true;
		}
		...
		std::string relSrc;
		int relDelta = 0;
		if (parseAddSubImmediate(line, targetBase, relSrc, relDelta) &&
		    relSrc == root) {
			targetOffset = offset - relDelta;
			return true;
		}

		if (!lineWritesReg(line, root))
			continue;

		if (line.mnemonic == "mov" && line.operands.size() == 2 &&
		    line.operands[0] == root && regClass(line.operands[1]) == 'x') {
			root = line.operands[1];
			if (root == targetBase) {
				targetOffset = offset;
				return true;
			}
		...
		std::string src;
		int delta = 0;
		if (parseAddSubImmediate(line, root, src, delta)) {
			root = src;
			offset += delta;
			if (root == targetBase) {
				targetOffset = offset;
...
static bool tryMachineMergeQPair(MachineBasicBlock &block, size_t idx) {
	if (idx + 1 >= block.instrs.size()) return false;
	...
	if (first.kind != LineKind::Instruction || second.kind != LineKind::Instruction)
		return false;
	if (first.operands.size() != 2 || second.operands.size() != 2)
		return false;
	if (!((first.mnemonic == "ldr" && second.mnemonic == "ldr") ||
	      (first.mnemonic == "str" && second.mnemonic == "str") ||
	      (first.mnemonic == "ld1" && second.mnemonic == "ld1") ||
	      (first.mnemonic == "st1" && second.mnemonic == "st1")))
		return false;

	const bool isLoad = first.mnemonic == "ldr" || first.mnemonic == "ld1";
	std::string firstReg = qMemReg(first);
	std::string secondReg = qMemReg(second);
	if (firstReg.empty() || secondReg.empty())
		return false;
	if (isLoad && samePhysicalReg(firstReg, secondReg))
		return false;
	if (!isLoad && !samePhysicalReg(firstReg, secondReg))
		return false;
	...
	MemOperand firstMem = parseMemOp(first.operands[1]);
	MemOperand secondMem = parseMemOp(second.operands[1]);
	if (!firstMem.valid || !secondMem.valid)
		return false;

	if (firstMem.base.empty() || firstMem.base == "sp")
		return false;

	int secondOffsetFromFirstBase = 0;
	if (!traceAddressToCurrentBase(block, idx + 1, secondMem,
	                               firstMem.base, secondOffsetFromFirstBase))
		return false;
	if (secondOffsetFromFirstBase - firstMem.offset != 16)
		return false;

	if (!validPairOffset(firstMem.offset, 16))
		return false;
	std::string addr = "[" + firstMem.base;
	if (firstMem.offset != 0)
		addr += ", #" + std::to_string(firstMem.offset);
	addr += "]";

	replaceMachineInstr(block.instrs[idx],
	                    makeMachineInsn(isLoad ? "ldp" : "stp",
	                                    {firstReg, secondReg, addr}));
	block.instrs.erase(block.instrs.begin() + idx + 1);
	return true;
}
