...
**调度**  
Pre-RA / Post-RA 共用 Cortex-A53 调度模型，但采用分阶段策略。模型描述双发射宽度、执行资源容量与占用周期，并按生产者和消费者的操作数类型计算旁路后的有效依赖延迟。cycle-aware list scheduler 根据 RAW/WAR/WAW、条件码和内存依赖构图；仅对可证明不相交的栈对象或直接全局对象解除访存依赖，未知地址仍按可能别名处理。Pre-RA 在同级候选间优先降低虚拟寄存器压力，并且仅在峰值压力或压力面积严格改善、不增加冲突边、且保持所有虚拟寄存器指令相对顺序时接受新顺序，避免局部指标误导后续图着色；Post-RA 使用最终物理寄存器依赖完成面向流水线的终端重排。

设置 `DEBUG_AARCH64_REWRITE_SCHEDULER=1` 可在 stderr 查看各调度区域的阶段、指令数、估算周期，以及调度前后的虚拟寄存器峰值/压力面积；未达到 Pre-RA 接受条件的候选会标为 `rejected`。
